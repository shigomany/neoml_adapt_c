...
namespace NeoML
{
	...
	CRandom::CRandom(unsigned int seed) : carry(initialCarry),
										  lagPosition(0)
	{
		Reset(seed);
	}
...
void* CRandomInitEmpty()
{
	return new CRandom();
}

void CRandomReset(void* ptr, unsigned int seed)
{
	auto instance = static_cast<CRandom *>(ptr);
	return instance->Reset(seed);
}

unsigned int CRandomNext(void* ptr)
{
	auto instance = static_cast<CRandom *>(ptr);
	return instance->Next();
}

double CRandomUniform(void* ptr, double min, double max) {
	auto instance = static_cast<CRandom *>(ptr);
	return instance->Uniform(min, max);
}

int CRandomUniformInt(void* ptr, int min, int max) {
	auto instance = static_cast<CRandom *>(ptr);
	return instance->UniformInt(min, max);
}

double CRandomNormal(void* ptr, double mean, double sigma) {
	auto instance = static_cast<CRandom *>(ptr);
